...
#[derive(Copy, Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, Enum)]
pub enum Warning {
    ///  replacing previously-defined element
    ReplacingElement,
    ///  too many unclosed tags
    ManyOutstandingTags,
    ///  argument needed but not supplied or blank
    ArgumentNotSupplied,
    ///  eg. </send bar >
    ArgumentsToClosingTag,
    ///  when closing an open tag secure tag blocks it
    ...
    TagOpenedInSecureMode,
    ///  eg. <b> <i> </b>
    ClosingOutOfSequenceTag,
    ///  opening tag no longer in buffer
    OpenTagNotInOutputBuffer,
    ///  no name known - cannot reply to <user>
    CharacterNameRequestedButNotDefined,
    ///  password requested too late in session
    PasswordNotSent,
    ///  no password known - cannot reply to <pass>
    PasswordRequestedButNotDefined,
    ///  MXP tag known but not implemented
    TagNotImplemented,
    ///  tag closed because \n received
    OpenTagClosedAtEndOfLine,
    ///  tag closed because <reset> received
    TagClosedAtReset,
    ///  argument supplied but not used, eg. <bold blue>
    UnusedArgument,
    ///  Pueblo string received but not activating it
    NotStartingPueblo,
}

impl ScriptArg for Warning {
    fn to_arg<'lua>(self, _: &'lua Lua) -> mlua::Result<Value<'lua>> {
        Ok(Value::Integer(self as i64 + 5_000))
    }
}

#[derive(Copy, Clone, Debug, PartialEq, Eq, PartialOrd, Ord, Hash, Enum)]
pub enum Info {
    ///  <version> response sent
    VersionSent,
    ///  <user> response sent
    CharacterNameSent,
    ///  <pass> response sent
    PasswordSent,
    ///  collecting <script> text
    ScriptCollectionStarted,
    ///  finished collecting <script>
    ScriptCollectionCompleted,
    ///  <reset> received
    ResetReceived,
    ///  mxp turned off
    Off,
    ///  mxp turned on
    On,
    ///  mxp security mode change
    ModeChange,
    ///  mxp <supports> tag sent
    SupportsSent,
    ///  mxp <options> tag sent
    OptionsSent,
    ///  mxp <afk> tag sent
    AFKSent,
    ///  client option changed by <recommend_option>
    OptionChanged,
}

impl ScriptArg for Info {
    fn to_arg<'lua>(self, _: &'lua Lua) -> mlua::Result<Value<'lua>> {
        Ok(Value::Integer(self as i64 + 10_000))
    }
}
